...
// The last 5 light levels we've seen
byte lightlevels[5];
// ----------------------------------------
...
void progSetup()
{
  ...
  pinMode(R1, OUTPUT);     
  pinMode(R2, OUTPUT);
  for (int i=0; i<5; i++) lightlevels[i] = 0;
}
...
void progMain()
{
  switch (mode)
  {
    ...
    case 1: // prints each character in string one at a time
      ...
      {
        cycle = 0;
        int ll = 4 - (osReadLight()/205);
        clearMatrix();
        for (int i=4; i>=0; i--) {
          lightlevels[i+1] = lightlevels[i];
          matrix[4-i][lightlevels[i]] = 1;
        }
        lightlevels[0] = ll;
        matrix[4][ll] = 1;
        analogWrite(10, osReadLight()/4);
        Serial.println(osReadLight()/4);
      }
      break;
...
void evButtons(int ID, bool state)
{
  if (state)
  {
    ...
    switch (mode)
    {
      case 0: charIDX = 0x5B; analogWrite(10, 250); break;
      case 1: charIDX = 0; break;
      case 2: charIDX = 0; break;
      case 3: charIDX = 0; break;              
    }
